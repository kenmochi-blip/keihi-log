...
const Gemini = (() => {
   ...
   */
  const _STATUS_HINT = {
    400: '[400] リクエスト不正（画像が破損しているか、対応していない形式の可能性があります）',
    401: '[401] APIキーが無効です。設定タブでGemini APIキーを再入力してください',
    403: '[403] APIキーに権限がありません。Google AI StudioでGemini APIが有効になっているか確認してください',
    404: '[404] モデルが見つかりません。管理者にお問い合わせください',
    429: '[429] APIのリクエスト上限に達しました。短時間に連続で読み取った場合は1分ほど待って再試行してください。1日待っても直らない場合は1日の上限到達です（/faq#q402b）',
    500: '[500] Geminiサーバーの内部エラーです。しばらく待ってから再試行してください',
    503: '[503] Geminiサーバーが一時的に過負荷です。しばらく待ってから再試行してください',
    504: '[504] タイムアウト：処理に時間がかかりすぎました。画像を小さくするか、時間をおいて再試行してください',
  };
...
- total_amount は「税込み費用計上額（源泉徴収控除前）」を入れること
  - 源泉徴収税が差し引かれている場合、合計欄の支払金額（源泉控除後）ではなく、小計＋消費税の合計を使う
    （例：小計33,000円・消費税3,300円・源泉徴収▲3,369円・合計32,931円 → total_amount = 36,300）
  - 源泉徴収額は withholding_amount に別途セットする
...
- tax_rate：食品・飲料なら「課税8%」、鉄道・バス・タクシー・交通系ICカード等の交通費は「課税10%」、非課税取引（医療・教育・住宅家賃等）なら「非課税」、不課税取引（給与・保険料等）なら「不課税」、複数税率混在なら「混在」、それ以外は「課税10%」
