...
<div class="pt-3">
  ...
  async function bindEvents(el, opts = {}) {
    ...
    if (!opts.fromCache && !_editId && App.isAdmin()) _loadTemplateQuickList(el);
...
function _bindSubtypePills(el) {
  ...
  async function _runAuditChecks(expenses, data, newHashes) {
    ...
    // 1.7. インボイス番号なしの高額仕入れ（判定は App.invoiceMissing に集約）
    if (App.invoiceMissing({ ...data, type: _currentType })) alerts.push(App.INVOICE_MISSING_MSG);
