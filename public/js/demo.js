...
const Demo = (() => {
  ...
  const EXPENSES = [
    // ── 2026-06 ──
    _e('demo-j01', '2026-06-01T09:00:00Z', 'デモ ユーザー', 'demo@example.com',
       '領収書', '2026-06-01', 'ルノアール 新宿店', 1080, '会議費', '取引先との打ち合わせ', false, 'T7000000000007',
       _img('receipt_renoir.svg')),
    _e('demo-j02', '2026-06-01T14:00:00Z', '田中 太郎', 'tanaka@example.com',
       '電車/バス', '2026-06-01', '東京→横浜（往復）', 940, '旅費交通費', '顧客訪問', false, ''),
    _e('demo-j03', '2026-06-01T10:30:00Z', '佐藤 次郎', 'sato@example.com',
       '領収書', '2026-06-01', 'ヨドバシカメラ', 4290, '消耗品費', 'SDカード・USBメモリ', false, 'T1000000000001',
       _img('receipt_yodobashi.svg')),
    _e('demo-j04', '2026-06-01T11:00:00Z', '鈴木 花子', 'suzuki@example.com',
       '領収書', '2026-06-01', 'アスクル', 2640, '消耗品費', '6月オフィス備品補充', false, 'T8000000000008',
       _img('receipt_askul.svg')),
    // 同じ領収書を2回撮ってしまったケース（重複検知アラートのデモ用）。
    // 同日・同額・同一取引先なので、実際の登録でも「重複の疑い」が付く組み合わせ。
    // 証票もあえて同一画像を添付し「同じ領収書の二重登録」を目で確認できるようにする。
    ...
    // ── 税率混在サンプル（1枚のレシートに課税10%・軽減8%品目が混在する例）──
    //   コンビニで「文具（消耗品費・課税10%）」と「会議用のお茶菓子（会議費・軽減税率8%）」を
    //   同時購入 → 明細分割モードで1レコードに2科目を登録するケース。
    //   category は "消耗品費/会議費" のようにスラッシュ区切りで保存される。
    _e('demo-sp1', '2026-05-01T11:00:00Z', '田中 太郎', 'tanaka@example.com',
       '領収書', '2026-05-01', 'セブン-イレブン 渋谷店', 1960, '消耗品費:880:課税10%/会議費:1080:課税8%',
       'ボールペン・付箋（課税10%）＋会議用お茶・菓子（軽減税率8%）', false, '',
       _img('receipt_seven.svg'), '', '混在'),
    // ── 2026-04 ──
    ...
    _e('demo-010', '2026-04-10T10:30:00Z', 'デモ ユーザー', 'demo@example.com',
       ...
       _img('receipt_coworking.svg'), '2026-05-10'),
    // ── 2026-03 ──
    ...
    _e('demo-c01', '2026-05-10T10:00:00Z', 'デモ ユーザー', 'demo@example.com',
       '領収書', '2026-05-10', 'AWS（クラウド利用料）', 38500, '通信費', '5月分クラウド費用', true, '',
       _img('receipt_aws.svg'), '会社払い（三井住友）'),
